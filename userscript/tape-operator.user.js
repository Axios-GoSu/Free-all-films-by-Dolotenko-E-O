...
// @updateURL       https://github.com/Axios-GoSu/Free-all-films-by-Dolotenko-E-O/raw/main/userscript/tape-operator.user.js
// @downloadURL     https://github.com/Axios-GoSu/Free-all-films-by-Dolotenko-E-O/raw/main/userscript/tape-operator.user.js
...
function extractMovieData() {
	...
	if (url.match(IMDB_MATCHER)) return { imdb: url.split('/').at(4), title };
	if (url.match(TMDB_MATCHER)) return { tmdb: url.split('/').at(4).split('-').at(0), title };
...
function extractTitle() {
	try {
		...
		if (title.includes('— смотреть онлайн в хорошем качестве — Кинопоиск')) title = title.replace('— смотреть онлайн в хорошем качестве — Кинопоиск', '').trim();
		if (title.includes('⭐')) title = title.split('⭐').at(0).trim();
...
function attachBanner() {
	...
	banner.addEventListener('mousedown', (e) => e.button === 1 && openPlayer(true));
	setTimeout(() => { banner.style.top = '-24px'; banner.style.opacity = '1'; }, 300);
